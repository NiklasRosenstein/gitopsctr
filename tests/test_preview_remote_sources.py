...
def _target_repository(tmp_path: Path, source: dict[str, object]) -> tuple[Path, str]:
    ...
    baseline = tmp_path / "target-empty-desired"
    baseline.mkdir()
    (baseline / ".gitkeep").write_text("")
    GitStateStore(target).publish("gitopsctr/desired/dev", baseline, None, "initialize desired state")
...
def _apply(root: Path, revision: str, monkeypatch: pytest.MonkeyPatch) -> tuple[str, bool]:
    monkeypatch.setattr(controller, "REPOSITORY_ROOT", root)
    controller._state_store.cache_clear()
    desired_ref = "gitopsctr/desired/dev"
    previous = controller.fetch_ref(desired_ref)
    stack = root / "deployment/environments/dev/stacks/application.json"
    ...
    desired = controller.command_apply(arguments)
    assert desired is not None
    return desired, desired != previous
...
def test_remote_ref_pins_commit_and_reconcile_ignores_moved_main(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    with _git_daemon(tmp_path, remote) as daemon:
        ...
        desired_a, changed_a = _apply(target, target_revision, monkeypatch)
        assert changed_a
        ...
        daemon.start()
        desired_b, changed_b = _apply(target, target_revision, monkeypatch)
        assert changed_b
...
def test_fixed_remote_commit_stays_pinned_after_main_moves(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    with _git_daemon(tmp_path, remote) as daemon:
        ...
        git(working, "push", "origin", "main")
        desired_b, changed_b = _apply(target, target_revision, monkeypatch)
        desired_c, changed_c = _apply(target, target_revision, monkeypatch)
...
def test_remote_source_failures_do_not_publish_apply_result(
    ...
    if failure == "fetch":
        ...
        initial_desired = GitStateStore(target).fetch("gitopsctr/desired/dev").revision
        with pytest.raises(OperationError, match=expected):
            _apply(target, target_revision, monkeypatch)
        ...
        with _git_daemon(tmp_path, remote) as daemon:
            ...
            with pytest.raises(OperationError, match=expected):
                _apply(target, target_revision, monkeypatch)
    assert controller.fetch_ref("gitopsctr/desired/dev") == initial_desired
