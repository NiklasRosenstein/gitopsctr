"""Path-free relationship runtime for logical workspace inspection."""
...
from gitopsctr.operational import (
    ...
    classify_workspace_before_observation,
...
class WorkspaceInventoryError(OperationError):
...
class WorkspaceInventoryRecord:
    ...
    def address(self) -> ResourceAddress:
        placement = WorkspaceInventorySession._placement(self.family, self.plane)
...
def _graph_parsed(record: WorkspaceInventoryRecord) -> object:
    ...
    if metadata is None or specification is None:
        raise WorkspaceInventoryError(
            f"desired {record.family.singular} {record.name!r} has no typed graph representation"
        )
...
class UnitOperationalState:
    unit: WorkspaceInventoryRecord
    observation: InventoryObservationState
    reconciliation: ReconciliationState
    reason: str
    receipt: WorkspaceInventoryRecord | None = None
    artifacts: tuple[ArtifactLink, ...] = ()


@dataclass(frozen=True)
class ReceiptOperationalState:
    receipt: WorkspaceInventoryRecord
    observation: InventoryObservationState
    unit: WorkspaceInventoryRecord | None
    artifacts: tuple[ArtifactLink, ...] = ()
...
class ArtifactOperationalState:
    ...
    artifact: WorkspaceInventoryRecord
    authentication: InventoryObservationState
    producer: WorkspaceInventoryRecord | None
    receipt: WorkspaceInventoryRecord | None
...
class ObservationOperationalState:
    ...
    resource: WorkspaceInventoryRecord
    observation: InventoryObservationState
    counterpart: WorkspaceInventoryRecord | None
...
class WorkspaceInventorySession:
    ...
    def __enter__(self) -> WorkspaceInventorySession:
        return self
    ...
    def resource_qualified_name(self, record: object) -> str:
        if not isinstance(record, WorkspaceInventoryRecord):
            raise WorkspaceInventoryError("resource address resolution requires an inventory record")
    ...
    def relationship_sources(self, relationship: str, target: object) -> tuple[WorkspaceInventoryRecord, ...]:
        if not isinstance(target, WorkspaceInventoryRecord):
            raise WorkspaceInventoryError("relationship address resolution requires an inventory record")
        ...
        if target.family.name != definition.target_family or target.plane is not definition.target_plane:
            raise WorkspaceInventoryError(f"relationship {relationship!r} does not target {target.family.name!r}")
        ...
        matches: list[WorkspaceInventoryRecord] = []
    ...
    def resources(
        ...
        *,
        ...
        if placement.scope.value == "environment" and environment is None:
            raise WorkspaceInventoryError(f"resource {family.plural!r} requires an environment")
        if placement.plane is ResourcePlane.SOURCE:
            if ref is not None or revision is not None:
                raise WorkspaceInventoryError("source resources do not accept ref or revision overrides")
            snapshot = self.planes.source()
        else:
            if ref is None:
                raise WorkspaceInventoryError(f"{placement.plane} resources require a ref")
            try:
                snapshot = self.planes.snapshot(placement.plane, ref, revision, allow_missing=allow_missing_ref)
            except OperationError as exc:
                raise WorkspaceInventoryError(str(exc)) from exc
    ...
    def _discover(
        ...
        try:
            discovered = discover_workspace_collection(
                collection,
            ...
            raise WorkspaceInventoryError(f"{location}{placement.plane} {family.plural}: {exc}") from exc
        ...
        identities: dict[ResourceAddress, WorkspaceInventoryRecord] = {}
    ...
    def _placement(family: ResourceFamilyDefinition, plane: ResourcePlane | None) -> ResourcePlacement:
        ...
        if len(candidates) != 1:
            ...
            raise WorkspaceInventoryError(f"resource family {family.name!r} has no unambiguous {description} placement")
        return candidates[0]

    def deployment_refs(self, environment: str) -> tuple[str, str]:
        records = self.resources("environment", environment=environment)
        if len(records) != 1:
            raise WorkspaceInventoryError(f"expected exactly one Environment resource for {environment!r}")
        specification = records[0].document.get("spec")
        refs = specification.get("refs", {}) if isinstance(specification, dict) else {}
        if not isinstance(refs, dict):
            raise WorkspaceInventoryError(f"Environment {environment!r} has invalid refs")
        ...
        if not isinstance(desired, str) or not isinstance(observed, str) or not desired or not observed:
            raise WorkspaceInventoryError(f"Environment {environment!r} has invalid desired or observed refs")
        if desired == observed:
            raise WorkspaceInventoryError(f"Environment {environment!r} desired and observed refs must differ")
    ...
    def environment_inventory(
        ...
        *,
        ...
        tuple[WorkspaceInventoryRecord, ...], tuple[WorkspaceInventoryRecord, ...], tuple[WorkspaceInventoryRecord, ...]
    ]:
    ...
    def evaluate_environment(
        ...
        *,
        ...
        if unit_view is None or unit_view.observation is None or unit_view.artifact_description is None:
            raise WorkspaceInventoryError("resource family 'unit' has no registered inspection relationships")
    ...
    def reconciliation_counts(self, environment: str) -> dict[ReconciliationState, int]:
        ...
        for path in cleanup_paths:
            ...
            previous = cleanup_by_name.get(name)
            if previous is not None:
                raise WorkspaceInventoryError(
                    f"environment {environment!r}, desired: duplicate opaque cleanup root {name!r} "
                    f"at {previous} and {path}"
                )
            cleanup_by_name[name] = path
    ...
    def stack_inspection_summary(self, record: InspectionRecord) -> StackInspectionSummary:
        ...
        if not isinstance(record, WorkspaceInventoryRecord):
            raise WorkspaceInventoryError("inspection presenter received a non-inventory record")
        if record.family.name not in {"stack", "stacktemplate"}:
            raise WorkspaceInventoryError(f"resource {record.name!r} has no Stack inspection summary")
        if record.environment is None or record.ref is None:
            raise WorkspaceInventoryError(f"resource {record.name!r} has no desired environment provenance")
    ...
    def prepare_stack_inspection(
        ...
        *,
        ...
        if any(
            ...
            raise WorkspaceInventoryError("Stack inspection records must share one desired snapshot")
    ...
    @staticmethod
    def _active_projection_bindings(record: WorkspaceInventoryRecord) -> tuple[StackProjectionUnitBinding, ...]:
    ...
    @staticmethod
    def _template_reference(record: WorkspaceInventoryRecord) -> tuple[str, str, str] | None:
        specification = getattr(record.parsed, "spec", None)
    ...
    @staticmethod
    def _raw_stack_names_for_templates(
        workspace: ImmutableWorkspace,
        templates: tuple[WorkspaceInventoryRecord, ...],
    ) -> frozenset[str]:
        fences = {
        ...
        names: set[str] = set()
        candidates = tuple(
            (PurePosixPath(entry.key), entry.content)
            for entry in workspace.list_entries("stacks")
            if entry.kind is WorkspaceEntryKind.FILE and entry.content is not None
        )
        for path, raw in candidates:
            if path.suffix.lower() not in {".yaml", ".yml", ".json"}:
                continue
            try:
                document = parse_document_bytes(raw, path)
            except Exception:
                continue
    ...
    def _build_stack_inspection_summaries(
        ...
        *,
        ...
        if first.environment is None or first.ref is None:
            raise WorkspaceInventoryError(f"resource {first.name!r} has no desired environment provenance")
        ...
        if first.family.name == "stack":
            ...
            templates = records
            stack_names = self._raw_stack_names_for_templates(first.snapshot.workspace, templates)
        ...
        units_by_identity: dict[tuple[str, str, str], list[WorkspaceInventoryRecord]] = {}
        units_by_binding: dict[tuple[str, str, str, str, str], WorkspaceInventoryRecord] = {}
        units_by_owner: dict[tuple[str, str, str, str], list[WorkspaceInventoryRecord]] = {}
        for unit in units:
        ...
        valid_units: list[WorkspaceInventoryRecord] = []
        ...
        def record_child_state(
            stack: WorkspaceInventoryRecord, binding: StackProjectionUnitBinding
        ) -> WorkspaceInventoryRecord | None:
            name = binding.name
    ...
    def _resource_partition(
        ...
        if not isinstance(metadata, dict):
            raise WorkspaceInventoryError(f"desired resource {record.name!r} has invalid metadata")
        ...
        if not isinstance(owners, list) or len(owners) != 1 or not isinstance(owners[0], dict):
            raise WorkspaceInventoryError(f"desired resource {record.name!r} has invalid owner references")
        owner = owners[0]
        ...
        if not all(isinstance(value, str) and value for value in (api_version, kind, name, uid)):
            raise WorkspaceInventoryError(f"desired resource {record.name!r} has an invalid owner identity")
        ...
        if key in visited:
            raise WorkspaceInventoryError(f"desired ownership cycle includes {api_version}/{kind} {name!r}")
        visited.add(key)
        try:
            owner_gvk = GVK(api_version, kind)
            owner_family = self.registry.family_for_api_kind(owner_gvk)
        except (KeyError, ValueError) as exc:
            raise WorkspaceInventoryError(f"desired resource {record.name!r} references an unregistered owner") from exc
        candidates = self.resources(
            owner_family.name,
        ...
        if len(owners_by_identity) != 1:
            raise WorkspaceInventoryError(
                f"desired resource {record.name!r} references a missing owner {owner_gvk} {name!r}"
            )
        ...
        if not isinstance(owner_metadata, dict) or owner_metadata.get("uid") != uid:
            raise WorkspaceInventoryError(f"desired resource {record.name!r} has a stale owner UID fence")
        return self._resource_partition(owner_record, visited)


def evaluate_relationships(
    registry: ResourceRegistry,
    units: tuple[WorkspaceInventoryRecord, ...],
    receipts: tuple[WorkspaceInventoryRecord, ...],
    ...
    *,
    ...
    address_runtime: WorkspaceInventorySession | None = None,
) -> RelationshipEvaluation:
        ...
        if len(unit_families) == 1:
            ...
            if observation is None:
                ...
                if len(observations) != 1:
                    raise WorkspaceInventoryError("registered Unit observation relationship is not unambiguous")
            ...
            if description is None:
                ...
                if len(descriptions) != 1:
                    raise WorkspaceInventoryError(
                        "registered Unit artifact-description relationship is not unambiguous"
                    )
    ...
    receipt_by_subject: dict[str, WorkspaceInventoryRecord] = {}
    ...
    for receipt in receipts:
        ...
        try:
            ...
            raise WorkspaceInventoryError(
                f"environment {receipt.environment!r}, observed {receipt.path}: {exc}"
            ) from exc
    ...
    if strict_artifacts and orphan_artifact_paths:
        ...
        raise WorkspaceInventoryError(
            f"environment {environment!r}: observed Artifacts are not described by a Receipt: {paths}"
        )
    ...
    for unit in units:
        ...
        if not isinstance(unit.parsed, UnitResource):
            raise WorkspaceInventoryError(f"desired Unit {unit.name!r} has no typed Unit representation")
        try:
            transition_reason = load_workspace_transition_blocks(unit.snapshot.workspace).get(unit.qualified_name)
            status = classify_workspace_before_observation(
                unit.snapshot.workspace,
                unit.qualified_name,
                unit.document,
                unit.parsed,
                transition_reason,
            ...
            raise WorkspaceInventoryError(f"environment {unit.environment!r}, desired {unit.path}: {exc}") from exc
...
def evaluate_observation_relationship(
    ...
    subjects: tuple[WorkspaceInventoryRecord, ...],
    observers: tuple[WorkspaceInventoryRecord, ...],
    ...
    observer_by_subject: dict[ResourceIdentity, WorkspaceInventoryRecord] = {}
    ...
    for observer in observers:
        ...
        try:
            ...
            raise WorkspaceInventoryError(
                f"environment {observer.environment!r}, observed {observer.path}: {exc}"
            ) from exc
